...
def upgrade_to_version_2():
    ...
    for service in my_services:
        ...
        for i in old_use:
            okc_name = 'okc-%s' % i
            if i not in all_templates and okc_name in all_templates:
...
def upgrade_to_version_2_1():
    ...
    for i in my_services:
        ...
        if 'check_command' not in i._defined_attributes:
            continue
        ...
        if check_command in all_commands:
            continue
    ...
    for host in my_hosts:
        ...
        for i in old_use:
            ...
            if i not in all_templates and okc_name in all_templates:
                # parent does not exist, but okc-parent does
                i = okc_name
            elif i not in all_templates and i in deprecated_hostnames:
                i = new_hostname
            new_use.append(i)
        # new_use = ','.join(new_use)
        if old_use != new_use:
...
def upgrade_to_version_2_2():
    ...
    for service in my_services:
        ...
        print(
            ...
            "renamed _SNMP_COMMUNITY %s to __SNMP_COMMUNITY" %
            (service['_SNMP_COMMUNITY']))
    print("ok")
...
def upgrade_to_version_2_3():
    ...
    my_services = Model.Service.objects.filter(
        check_command="okc-emc-check_portstate", __USERNAME=None, register=1)
    for i in my_services:
    ...
    my_services = Model.Service.objects.filter(
        check_command="okc-check_http", __VIRTUAL_HOST=None, register=1)
    for i in my_services:
    ...
    my_services = Model.Service.objects.filter(
        check_command="okc-check_https", __VIRTUAL_HOST=None, register=1)
    for i in my_services:
        if i.get('__VIRTUAL_HOST') is None:
            i['__VIRTUAL_HOST'] = i.get('host_name')
        if i.get('__PORT') is None:
    ...
    my_services = Model.Service.objects.filter(
        check_command="okc-check_https_certificate", __PORT=None, register=1)
    for i in my_services:
        if i.get('__PORT') is None:
            i['__PORT'] = "443"
        i.save()
...
def upgrade_to_version_2_5():
    ...
    * okc-check_http* has a new service variable __ON_REDIRECT
    ...
    services = Model.Service.objects.filter(
        check_command__startswith='okc-check_http',
        __ON_REDIRECT__exists=False)
    for i in services:
...
def rename_oktemplate_services():
    ...
    all_obj = Model.Service.objects.filter(
        name__contains='',
        filename__startswith=okconfig.template_directory)
    for i in all_obj:
        if i.name.startswith('okc-'):
            continue
        i['name'] = "okc-%s" % i.name
        i.save()
