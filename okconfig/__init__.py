...
def discover_inet(host_name, service):
    # Check if we have already checked this
    if discovery_cache['inet'].has_key("%s,%s" % (host_name, service)):
        return discovery_cache['inet']["%s,%s" % (host_name, service)]
    ...
    # Let's cache it
    discovery_cache['inet']["%s,%s" % (host_name, service)] = scan_result
